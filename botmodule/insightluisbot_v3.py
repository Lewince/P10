...
class InsightLuisBot_v3(ActivityHandler):
    
    def __init__(self, constate:ConversationState, userstate:UserState,
    appid, appkey, exporter, logger, verbose=False):
        ...
        # Instantiating appinsights metrics and monitoring helpers
        self.duration_measure = measure_module.MeasureFloat("duration",
                                                "conversation duration",
                                                "seconds")
        self.duration_view = view_module.View("duration view",
                                    "conversation duration",
                                    [],
                                    self.duration_measure,
                                    aggregation_module.LastValueAggregation())
        self.processed_requests_measure = measure_module.MeasureInt("processed requests",
                                                "count of processed_requests",
                                                "total requests")
        self.processed_requests_view = view_module.View("request count",
                                    "request count",
                                    [],
                                    self.processed_requests_measure,
                                    aggregation_module.SumAggregation())
        self.successful_requests_measure = measure_module.MeasureInt("successful requests",
                                                "count of successful_requests",
                                                "requests")
        self.successful_requests_view = view_module.View("succesful request count",
        "request count",
        [],
        self.successful_requests_measure,
        aggregation_module.SumAggregation())
        self.user_failed_requests_measure = measure_module.MeasureInt("user-failed requests",
        "count of user notified understanding issues",
        "failures")
        self.user_failed_requests_view = view_module.View("user notified failure count",
        "user found failure count",
        [],
        self.user_failed_requests_measure,
        aggregation_module.SumAggregation())
        self.luis_failed_requests_measure = measure_module.MeasureInt("luis_failed_requests",
        "count of luis_failed_requests",
        "requests")
        self.luis_failed_requests_view = view_module.View("luis notified failure count",
        "luis failure count",
        [],
        self.luis_failed_requests_measure,
        aggregation_module.SumAggregation())
        self.view_manager.register_view(self.duration_view)
        self.view_manager.register_view(self.processed_requests_view)
        self.view_manager.register_view(self.successful_requests_view)
        self.view_manager.register_view(self.user_failed_requests_view)
        self.view_manager.register_view(self.luis_failed_requests_view)
    ...
    async def on_members_added_activity(self,
        ...
        self.start_timer = datetime.datetime.utcnow()
        self.mmap.measure_int_put(self.processed_requests_measure, 1)
    ...
    async def process_intents(self, conmode, usermode, turn_context, result_dict):
        if result_dict['intents']['Greet']['score'] >= 0.9:
            ...
            self.end_timer = datetime.datetime.utcnow()
            self.mmap.measure_int_put(self.successful_requests_measure, 1)
            self.mmap.record(self.tmap)
            if (self.end_timer) and (self.start_timer) : 
                conv_duration = self.end_timer - self.start_timer
                self.mmap.measure_float_put(self.duration_measure, conv_duration.total_seconds())
                self.mmap.record(self.tmap)
            ...
            self.mmap.measure_int_put(self.user_failed_requests_measure, 1)
            self.mmap.record(self.tmap)
        ...
        # Extract entities from results dict if intent is "book"
            ...
            await self.extract_entities(result_dict, usermode, conmode, turn_context)
            # Send summary
            if self.verbose:
            ...
            self.mmap.measure_int_put(self.luis_failed_requests_measure, 1)
            self.mmap.record(self.tmap)
            await turn_context.send_activity("I did not understand that, can you please rephrase?")
            self.logger.error("Unclear Intent from Recognizer - Lower P thresholds or improve Language Understanding Model")

    # Extract Intents and entities from results dict - log error message if issue with LU results
    async def extract_entities(self, result_dict, usermode, conmode, turn_context):
