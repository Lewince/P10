...
class Test_Bot_Activities_Test(aiounittest.AsyncTestCase):
    ...
    connection_string=self.CONFIG.INSIGHTS_CSTRING)
        self.bot = InsightLuisBot_v3(constate, userstate, self.CONFIG.LUIS_APP_ID, self.CONFIG.LUIS_KEY, exporter, logger)
    ...
    async def test_n_element_rejection_test(self):
        adapter = TestAdapter(self.bot.on_turn)
        await adapter.test("No", "I'm sorry I could not understand your request properly - Can you please rephrase that?")
    ...
    async def test_p_element_start_over_test(self):
        adapter = TestAdapter(self.bot.on_turn)
        await adapter.test("start over!", "OK let us start this all over : can you please give me some details about the trip you are searching for?")
